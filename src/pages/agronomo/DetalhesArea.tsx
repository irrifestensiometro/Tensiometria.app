  ...
  if (!area) {
    return (
      <div className="flex flex-col items-center justify-center p-12">
        ...
        <button onClick={() => navigate('/agronomo/dashboard')} className="mt-4 text-[#356b46] font-bold">Voltar</button>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto space-y-6 animate-fade-in">
      ...
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center space-x-4">
          <button
            onClick={() => navigate('/agronomo/dashboard')}
        ...
        <div className="flex items-center space-x-3">
          <button
            onClick={handleEdit}
            className="flex items-center space-x-2 bg-white hover:bg-slate-50 text-slate-700 px-5 py-2.5 rounded-xl font-bold transition-colors border border-slate-200 shadow-sm"
          >
            <Edit3 size={16} />
            <span>Editar</span>
          </button>
          <button
            ...
            className="flex items-center space-x-2 bg-red-50 hover:bg-red-100 text-red-700 px-5 py-2.5 rounded-xl font-bold transition-colors border border-red-200"
          >
            <Trash2 size={16} />
            <span>Excluir</span>
          </button>
      ...
      {showDeleteConfirm && (
        <div className="fixed inset-0 bg-slate-900/40 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md p-6 animate-in zoom-in-95 duration-200">
            <h2 className="text-xl font-bold text-slate-800 mb-2">Excluir Área</h2>
            <p className="text-slate-500 mb-6">
              Tem certeza que deseja excluir <strong>{area.nome}</strong>? Esta ação não pode ser desfeita e todas as leituras associadas serão perdidas.
            </p>
        ...
        <div className="lg:col-span-2 space-y-6">
            ...
            <div className="flex items-center space-x-4">
              ...
              <div>
                ...
                <p className="text-xs text-slate-400 mt-1">
                  {temLeituraHoje ? 'Leitura de hoje' : ultimaLeitura ? 'Última leitura: ' + format(parseISO(ultimaLeitura.data), "dd/MM HH:mm", { locale: ptBR }) : 'Sem leituras'}
                </p>
              </div>
            </div>
            <div className="bg-white/60 p-4 rounded-xl border border-white/40 flex-1 w-full md:w-auto">
              <p className="text-slate-700 font-medium text-sm leading-relaxed">{recomendacao}</p>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
              <Gauge size={20} className="mr-2 text-slate-500" />
              Parâmetros Técnicos
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Coef. A (a)</p>
                <p className="font-bold text-slate-800 text-lg">{area.solo.coef_a.toFixed(3)}</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Coef. B (b)</p>
                <p className="font-bold text-slate-800 text-lg">{area.solo.coef_b.toFixed(3)}</p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Umidade CC (θcc)</p>
                <p className="font-bold text-slate-800 text-lg">{area.solo.umidade_cc.toFixed(2)} <span className="text-sm font-medium text-slate-500">cm³/cm³</span></p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Prof. Raiz (Z)</p>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Vazão (Ip)</p>
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">PAM</p>
                <p className="font-bold text-slate-800 text-lg">{(area.irrigacao.pam * 100).toFixed(0)} <span className="text-sm font-medium text-slate-500">%</span></p>
              </div>
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
              ...
              <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                <p className="text-[10px] uppercase font-bold text-slate-400 mb-1">Setores</p>
                <p className="font-bold text-slate-800 text-lg">{new Set(area.tensiometros.map(t => t.setor)).size}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm">
            <h3 className="text-lg font-bold text-slate-800 mb-4 flex items-center">
              ...
              Tensiômetros por Setor
            </h3>
            <div className="space-y-4">
              {Array.from(new Set(area.tensiometros.map(t => t.setor))).map(setor => {
                const tensiometrosSetor = area.tensiometros.filter(t => t.setor === setor);
                return (
                  <div key={setor} className="bg-slate-50 rounded-xl p-4 border border-slate-100">
                    <h4 className="font-bold text-slate-700 text-sm mb-3 uppercase tracking-wider">{setor}</h4>
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                      {tensiometrosSetor.map(t => {
                        const ultimoValor = ultimaLeitura?.valores.find(v => v.tensiometro_id === t.id);
                        return (
                          <div key={t.id} className="bg-white rounded-lg p-3 border border-slate-200 text-center">
                            <p className="text-[10px] uppercase font-bold text-slate-400">{t.prof_cm} cm</p>
                            <p className="text-lg font-black text-slate-700 mt-1">
                              {ultimoValor ? `${ultimoValor.leitura_kpa}` : '--'}
                              <span className="text-[10px] font-medium text-slate-400 ml-0.5">kPa</span>
                            </p>
                            {t.is_controle && (
                              <span className="text-[10px] font-bold text-blue-600 bg-blue-50 px-1.5 py-0.5 rounded-full">Controle</span>
                            )}
                          </div>
                        );
                      })}
        ...
        <div className="bg-white p-6 rounded-2xl border border-slate-200 shadow-sm h-fit">
          ...
          {leiturasArea.length === 0 ? (
            <div className="text-center py-8 text-slate-500">
              ...
              <p className="text-sm">Nenhuma leitura registrada.</p>
            </div>
          ) : (
                ...
                return (
                  <div key={leitura.id} className={`p-4 rounded-xl border ${index === 0 ? 'bg-slate-50 border-slate-200 shadow-sm' : 'border-slate-100 hover:bg-slate-50 transition-colors'}`}>
                    <div className="flex justify-between items-center mb-3">
                      ...
                      {index === 0 && (
                        <span className="text-[10px] bg-slate-200 text-slate-700 font-bold px-2 py-0.5 rounded-full uppercase">Recent</span>
                      )}
                    </div>
                      ...
                      {leitura.valores.map(v => {
                        ...
                        return (
                          <div key={v.tensiometro_id} className="flex justify-between items-center text-sm">
                            <span className="text-slate-500 font-medium">{tensiometro?.setor || '?'} - {tensiometro?.prof_cm || '?'}cm</span>
